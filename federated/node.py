...
from federated.oort.oort_wrapper import OortClientSampler


class Node:
    """Base node class without encryption."""

    def __init__(self, rank: int) -> None:
        """Initialize the node with its rank and device."""
        self.rank = rank
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self._ckpt = None
        self._ckpt_reparam = None
    ...
    def get_device_info(self) -> None:
        ...
        if self.device == 'cuda':
            device_id = torch.cuda.current_device()
            properties = torch.cuda.get_device_properties(device_id)
            print(f'Node {self.rank}: '
                  f'GPU {device_id} ({properties.name}) '
                  f'with {properties.total_memory / 1e9:.1f}GB memory')
        else:
            print(f'Node {self.rank}: Using CPU')

    def reparameterize(self, architecture: str = 'yolov7') -> None:
        """Reduce trainable modules into deploy model."""
        ckpt = copy.deepcopy(self._ckpt)
        backup_hyp = ckpt['model'].hyp
        backup_gr = ckpt['model'].gr
        nc = ckpt['model'].nc
        ...
        model.gr = backup_gr
        self._ckpt_reparam = {
            'model': copy.deepcopy(model.module if is_parallel(model) else model).half(),
            'optimizer': None,
            'training_results': None,
            'epoch': -1
        }
    ...
    def post_init_update(self, data: str, cfg: str, hyp: str, imgsz: int) -> None:
        """Post-initialization update of model."""
        ckpt = copy.deepcopy(self._ckpt)
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, 
                     anchors=hyp_dict.get('anchors')).to(self.device)
    ...
    def test(self, kround: int, saving_path: str, data: str, bsz: int, 
             imgsz: int, conf: float, iou: float) -> None:
...
class Server(Node):
    ...
    def get_weights(self, metadata: bool) -> dict:
        ...
        if metadata:
            return copy.deepcopy(self._ckpt)
        else:
            return copy.deepcopy(self._ckpt['model']).half().state_dict()

    def __compute_pseudo_gradient(self, updates: list, nsamples_list: list) -> dict:
        ...
        for key in delta_t.keys():
            delta_it_weighted = [
                delta_it[key] * (ni / n) 
                for delta_it, ni in zip(updates, nsamples_list)
            ]
            delta_t[key] = torch.sum(torch.stack(delta_it_weighted), dim=0)
    ...
    def __fedavg(self, delta_t: dict) -> dict:
        ...
        w_t = copy.deepcopy(self._ckpt['model'].state_dict())
        for key in w_t.keys():
            w_t[key] = w_t[key] - self.server_lr * delta_t[key]
        return w_t
    ...
    def __fedyogi(self, delta_t: dict) -> dict:
        ...
        self.v_t = {
            key: self.v_t[key] - (1. - self.beta2) * delta_t[key] ** 2 * 
                 torch.sign(self.v_t[key] - delta_t[key] ** 2)
            for key in delta_t.keys()
        }
        w_t = {
            key: w_t[key] - self.server_lr * self.m_t[key] / (torch.sqrt(self.v_t[key]) + self.tau)
    ...
    def aggregate(self, updates: list, nsamples_list: list) -> None:
        ...
        if self.server_opt == 'fedavg':
            ...
            raise ValueError(f'Unknown optimizer: {self.server_opt}')
...
class Client(Node):
    """Client node for local training."""

    def __init__(self, rank: int) -> None:
        """Initialize client with rank > 0."""
        if rank <= 0:
            raise ValueError('Client rank must be > 0')
        super().__init__(rank)
        self.__update = None
        self.nsamples = None
    ...
    def extract_losses(self, saving_path: str, epochs: int, nrounds: int) -> float:
        ...
        Args:
            ...
            nrounds: Current round number (1-indexed)
